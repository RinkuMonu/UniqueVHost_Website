...
export default function VpsServersPage() {
  const plans = [
    {
      title: "VPS Basic",
      description: "Ideal for personal projects and small websites",
      cpu: "1 vCPU",
      ram: "2 GB RAM",
      storage: "25 GB SSD",
    ...
    {
      title: "VPS Premium",
      description: "Optimized for high-traffic and resource-intensive apps",
      cpu: "4 vCPUs",
      ram: "8 GB RAM",
      storage: "100 GB SSD",
        ...
        <div className="container mx-auto px-4 relative z-10 grid md:grid-cols-2 items-center gap-12">
          <div>
            ...
            <p className="text-lg md:text-xl text-[#313149] max-w-xl mb-8">
              Experience unparalleled speed and reliability with our virtual private servers, designed for demanding workloads and global reach. Deploy in minutes with full root access.
            </p>
        ...
        <div className="grid lg:grid-cols-3 gap-10">
          ...
          <div className="lg:col-span-3 grid md:grid-cols-2 xl:grid-cols-3 gap-6">
              ...
              <Card className="sticky top-24 bg-white rounded-xl shadow-xl border-0 overflow-hidden transition-all duration-300">
                ...
                <CardContent className="space-y-6">
                  <div>
                    ...
                    <div className="space-y-2" style={{ display: "grid", gap: "10px" }}>
                      {["US East (N. Virginia)", "US West (Oregon)", "EU West (Ireland)", "Asia Pacific (Singapore)", "Canada (Central)"].map((region, idx) => (
                        <Label key={idx} className="flex items-center gap-2 font-normal text-sm text-[#4C5671]" style={{ display: "flex" }}>
                          <Checkbox className="border-[#4C5671] data-[state=checked]:bg-[#FD5D07] rounded-full mr-2" />
                          {region}
                        </Label>
                      ))}
                    </div>
            ...
            {plans.map((plan, idx) => (
              <Card
                key={idx}
                className={`relative bg-white bg-gradient-to-br from-[#fff] via-[#fdf7f1] to-[#fff8f4] rounded-2xl shadow-xl hover:shadow-2xl transition-all hover:-translate-y-2 hover:scale-[1.02] border ${plan.badge === "Recommended" ? "border-[#FD5D07]" : "border-gray-200"
                  }`}
              >
                {plan.badge && (
                  ...
                  {!plan.custom ? (
                    <>
                      <div className="space-y-2 text-[#4C5671]">
                        <div className="flex items-center gap-2"><Cpu className="w-4 h-4 text-[#FD5D07]" /> {plan.cpu}</div>
                        <div className="flex items-center gap-2"><MemoryStick className="w-4 h-4 text-[#FD5D07]" /> {plan.ram}</div>
                        <div className="flex items-center gap-2"><HardDrive className="w-4 h-4 text-[#FD5D07]" /> {plan.storage}</div>
                        <div className="flex items-center gap-2"><Globe className="w-4 h-4 text-[#FD5D07]" /> {plan.transfer}</div>
                      </div>
                      <Separator />
                      <div>
          ...
          <Card className="relative mt-16 bg-gradient-to-br from-[#FFF8F4] to-[#FFFFFF] rounded-2xl shadow-xl border-0 overflow-hidden">
            ...
            <CardContent className="relative z-10">
              <div className="grid md:grid-cols-3 gap-8">
                <div className="group text-center bg-white rounded-xl  p-6 hover:shadow-xl transition-all border-transparent hover:border-[#FD5D07]/20">
                  <div className="w-14 h-14 bg-[#FD5D07]/10 rounded-full flex items-center justify-center mx-auto mb-4">
                    <Cpu className="w-7 h-7 text-[#FD5D07]" />
                  </div>
                  <h3 className="font-semibold mb-2 text-[20px] text-[#001233] group-hover:text-[#FD5D07] transition-colors">
                    Blazing Fast SSDs
                  </h3>
                ...
                <div className="group text-center bg-white rounded-xl  p-6 hover:shadow-xl transition-all border-transparent hover:border-[#FD5D07]/20">
                  <div className="w-14 h-14 bg-[#001233]/10 rounded-full flex items-center justify-center mx-auto mb-4">
                    <MemoryStick className="w-7 h-7 text-[#001233]" />
                  </div>
                  <h3 className="font-semibold mb-2 text-[20px] text-[#001233] group-hover:text-[#FD5D07] transition-colors">
                    Dedicated Resources
                  </h3>
                ...
                <div className="group text-center bg-white rounded-xl  p-6 hover:shadow-xl transition-all border-transparent hover:border-[#FD5D07]/20">
                  <div className="w-14 h-14 bg-[#FFC107]/10 rounded-full flex items-center justify-center mx-auto mb-4">
