...
export default function Plans({ sharedHosting }: { sharedHosting: string }) {
  ...
  const getDurationMultiplier = (duration: string) =>
    duration === "monthly" ? 1 : duration === "yearly" ? 12 : 36;

  const filteredPlans = plans.filter((p) => p.duration === "monthly");
  ...
  const renewLine = (price: number, dur: string) =>
    `Renews at ${priceDisplay(price)}/${
      dur === "monthly" ? "mo" : dur === "yearly" ? "yr" : "3yr"
    }`;
  ...
  const handleBuyNow = async (planId: string) => {
    ...
    try {
       await axiosInstance.post("/orders/order/createorder", {
        user_id: userId,
            ...
            {filteredPlans.map((plan) => {
              ...
              return (
                <div
                  ...
                  className={`bg-white rounded-2xl shadow-xl hover:shadow-2xl transition-all p-6 text-center border hover:-translate-y-2 cursor-pointer ${
                    isSelected
                      ? "border-[#FD5D07] ring-2 ring-[#FD5D07] ring-opacity-20"
                      : "border-transparent hover:border-[#FD5D07]"
                  }`}
                >
                  <h3 className="font-bold text-xl mb-2 text-[#001233]">
                    {plan.name}
                  </h3>
                  <p className="text-sm text-gray-500 mb-4">
                    {renewLine(adjustedPrice, activeTab)}
                  </p>
          ...
          <div className="bg-white rounded-2xl shadow-xl p-8 border border-[#FD5D07] animate-in slide-in-from-top-4 duration-500">
            ...
            <div className="grid md:grid-cols-2 gap-8">
              {["Plan Features", "Email Features"].map((grp) => (
                <div key={grp}>
                  <h4 className="font-semibold text-lg mb-4 text-[#001233] border-b-2 border-[#FD5D07] pb-2">
                    {grp}
                  </h4>
